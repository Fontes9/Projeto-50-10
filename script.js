...
document
  ...
  .addEventListener("submit", function (e) {
    ...
    const coachText =
      document.getElementById("coachType").options[
        document.getElementById("coachType").selectedIndex
      ].text;
    const formatText =
      document.getElementById("classFormat").options[
        document.getElementById("classFormat").selectedIndex
      ].text;
...
function confirmCompletion(elementId, btnElement) {
  ...
  if (btnElement) {
    ...
    parentTd.innerHTML =
      '<span style="color: #d35400; font-weight: bold; font-size: 0.85rem;">Validado (Prof/Aluno)</span>';
  }

  alert(
    "Aula confirmada por ti!\nNotificação enviada à coordenação para validação final e fecho da faturação.",
  );
}
...
function newAccessory() {
  alert("Página de criar novo acessório"); //   talvez um página com um forms aqui
}
